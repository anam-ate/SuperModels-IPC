...
def main():
    ...
    with open(coldata1, 'wb') as f:
        ...
        while c4d.BaseTime(frame,fps) < doc.GetMaxTime():
            ...
            newSplitList = [allcol[i:i+1200] for i in xrange(0, len(allcol),1200)]  #60 pixels  x 4  (rgbw) = 240 values per frame

            for lst2 in newSplitList:
                for i, item in enumerate(lst2):
                    col_str = str( item) #must be string in csv clamp stops negatives from occuring  export as int
